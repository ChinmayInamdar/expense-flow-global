...
export function UploadSection({ apiKey, baseCurrency, onUploadComplete }: UploadSectionProps) {
  ...
  const processFiles = async () => {
    if (!apiKey) {
      toast({
        ...
        description: "Please enter your Google AI Studio API Key in the configuration panel",
        variant: "destructive",
      })
      return
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {files.length > 0 && (
        <div className="space-y-4">
          <Button onClick={processFiles} disabled={uploading || !apiKey} className="w-full" size="lg">
            {uploading ? (
              <>
      ...
      {!apiKey && (
        <Alert>
          <AlertDescription>
            Please configure your Google AI Studio API Key in the configuration panel to enable receipt processing.
            <br />
            <strong>For demo purposes:</strong> You can enter any text as the API key to test with mock data.
          </AlertDescription>
        </Alert>
      )}
    </div>
